...
class Model_penempatan extends CI_Model {
  ...
  */
  ...
  public function get_provinsi_kosong(){
    ...
    FROM(SELECT formasi.id  AS id_1 ,formasi.se,formasi.sk,formasi.ks, formasi.deskripsi, COUNT(sipaju_mahasiswaa.nim) AS terisi_ks FROM `formasi` LEFT JOIN sipaju_mahasiswaa ON formasi.id = sipaju_mahasiswaa.pil_fix WHERE sipaju_mahasiswaa.prodi = 'KS'GROUP BY formasi.id) AS tabel_1
    JOIN
    (SELECT formasi.id AS id_2, COUNT(sipaju_mahasiswaa.nim) AS terisi_se FROM `formasi` LEFT JOIN sipaju_mahasiswaa ON formasi.id = sipaju_mahasiswaa.pil_fix WHERE sipaju_mahasiswaa.prodi = 'SE'GROUP BY formasi.id) AS tabel_2
    ON id_1 = id_2
    JOIN
    (SELECT formasi.id AS id_3, COUNT(sipaju_mahasiswaa.nim) AS terisi_sk FROM `formasi` LEFT JOIN sipaju_mahasiswaa ON formasi.id = sipaju_mahasiswaa.pil_fix WHERE sipaju_mahasiswaa.prodi = 'SK'GROUP BY formasi.id) AS tabel_3
    ON id_1 = id_3
